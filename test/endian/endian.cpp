...
BOOST_AUTO_TEST_CASE(uint32) {
  ...
  for (const auto& tc : uint32_test_cases) {
    auto array = shiguredo::mp4::endian::uint32_to_be(tc.number);
    BOOST_REQUIRE_EQUAL_COLLECTIONS(std::begin(tc.array), std::end(tc.array), std::begin(array), std::end(array));
    auto number = shiguredo::mp4::endian::be_to_uint32(array);
    BOOST_REQUIRE_EQUAL(tc.number, number);
    auto number2 = shiguredo::mp4::endian::be_to_uint32(array[0], array[1], array[2], array[3]);
...
BOOST_AUTO_TEST_CASE(uint64) {
  ...
  for (const auto& tc : uint64_test_cases) {
    auto array = shiguredo::mp4::endian::uint64_to_be(tc.number);
    BOOST_REQUIRE_EQUAL_COLLECTIONS(std::begin(tc.array), std::end(tc.array), std::begin(array), std::end(array));
    auto number = shiguredo::mp4::endian::be_to_uint64(array);
    BOOST_REQUIRE_EQUAL(tc.number, number);
    auto number2 = shiguredo::mp4::endian::be_to_uint64(array[0], array[1], array[2], array[3], array[4], array[5],
                                                        array[6], array[7]);
    BOOST_REQUIRE_EQUAL(tc.number, number2);
  }
}
